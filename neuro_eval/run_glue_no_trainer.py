...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Where to store the pre-trained models downloaded from huggingface",
    )
    parser.add_argument(
        "--data_cache_dir",
        type=str,
        default=None,
        help="Where to store the datasets downloaded from huggingface",
    )
    parser.add_argument(
        "--use_mase",
        type=int,
        default=0,
        help="Whether to enable MASE transformation",
    )
...
def main():
    ...
    from spike_quan_wrapper_mase import MASE_SNNWrapper
    ...
    if args.use_mase == 1:
        ...
        if os.path.exists(os.path.join(args.model_name_or_path, "model.safetensors")):
            state_dict = load_file(os.path.join(args.model_name_or_path, "model.safetensors"))
    ...
    model, _ = ann2snn_module_transform_pass(
        model, quan_pass_args
    )
    quan_pass_args = {
        "by": "type",
        "embedding": {
    ...
    for step, batch in enumerate(eval_dataloader):
        with torch.no_grad():
            # outputs = model(**batch)
            breakpoint()
            outputs, count, accu_per_timestep = model(**batch)
