...
from utils.readData import get_train_data_loader,get_test_data_loader
...
def finetune(args):
    ...
    best_acc = 0.0
    finetune_epochs = min(args.epochs, 100)  # Use fewer epochs for fine-tuning
    
    for epoch in tqdm(range(finetune_epochs), desc="Fine-tuning"):
        train_loss = 0
        train_correct = 0
        for images, labels in trainloader:
            images = images.to(device)
